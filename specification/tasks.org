...
- remove fluff, be succinct
- explain `prototype` isn't prototypical inheritance, it's like cloning and modifying (dictionary definition), with no link to the parent/original
- lexical and dynamic scope example 
  - should lexical binding be the default for safety?
  - explain how `bindings` in fn lists affect their scope
