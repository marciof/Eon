...
- pass indented block as a list of functions? Let each function decide what to do.
  - pass each line still as a single argument 
  - a varargs arg gets all args in a list, `then...`
  - a dangling keyword arg gets all args in a list, `then=`
  - issue: ambiguous with passing an actual list? Or not an issue? Not an issue if a varargs arg, since it'll always get a list?
