...
object make_string_of_size(long len, int zero) {
	long i;
	object s = make_heap_object(
		STRING_TYPE, sizeof(struct string_heap_structure) + len + 1);

    STRING_LENGTH(s) = len;

    if (zero)
		for (i = 0; i <= len; i++) STRING_VALUE(s)[i] = '\0';
	else
		STRING_VALUE(s)[len] = '\0';
