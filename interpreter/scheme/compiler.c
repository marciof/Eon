...
static void compile_local_ref(long i, long j) {
    if (i <= 3) {
        ...
        case 0:
            switch (j) {
            case 0:
                encode_op(OPCODE_LOCAL00);
                return;
            case 1:
                encode_op(OPCODE_LOCAL01);
                return;
            default:
                encode_op(OPCODE_LOCAL0);
                break;
            }
            break;
        case 1:
            ...
            encode_op(OPCODE_LOCAL1);
            break;
        case 2:
            encode_op(OPCODE_LOCAL2);
            break;
        case 3:
            encode_op(OPCODE_LOCAL3);
            break;
        default:
            fatal_error(
                "Unknown opcode: compiler.c#compile_local_ref(): %lu\n", i);
        }
        encode(j);
    } else {
...
static void compile_ifelse(object pred, object consequent, object antecedent,
    ...
    antecedent = PAIR_P(antecedent) ? CAR(antecedent) : void_object;
