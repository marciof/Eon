...
static void prepare(struct e_Log* log, enum e_Log_Level level) {
    struct e_Log_FILE* file = reinterpret_cast<struct e_Log_FILE*>(log->val);
...
static void print_ch(struct e_Log* log, char ch) {
    fputc(ch, reinterpret_cast<struct e_Log_FILE*>(log->val)->stream);
}

static void print_str(struct e_Log* log, const char* str) {
    fputs(str, reinterpret_cast<struct e_Log_FILE*>(log->val)->stream);
}
...
struct e_Log* e_Log_get() {
    ...
    if (log == NULL) {
        log = reinterpret_cast<struct e_Log*>(mem);
        log->prepare = prepare;
        log->print_ch = print_ch;
        log->print_str = print_str;
        reinterpret_cast<struct e_Log_FILE*>(log->val)->stream = stdout;
    }
