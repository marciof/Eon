...
void e_Log_prepare(e_Log log, enum e_Log_Level level) {
    struct e_Log_FILE* file = static_cast<struct e_Log_FILE*>(log);
...
void e_Log_print_ch(e_Log log, char ch) {
    fputc(ch, static_cast<struct e_Log_FILE*>(log)->stream);
}

void e_Log_print_str(e_Log log, const char* str) {
    fputs(str, static_cast<struct e_Log_FILE*>(log)->stream);
}
