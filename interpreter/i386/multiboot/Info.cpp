...
void e_Multiboot_Info::log() {
    e_Log_get()->info("Multiboot: flags={iub}", this->flags);

    if (IS_FLAG_SET(this->flags, MULTIBOOT_INFO_MEMORY)) {
        e_Log_get()->info("Memory: lower={iu} KiB; upper={iu} KiB",
    ...
    if (IS_FLAG_SET(this->flags, MULTIBOOT_INFO_CMDLINE)) {
        e_Log_get()->info("Command line: \"{s}\"", this->cmdline);
    }
    ...
    if (IS_FLAG_SET(this->flags, MULTIBOOT_INFO_CONFIG_TABLE)) {
        ...
        e_Log_get()->info("ROM configuration: table={iuh}",
            this->config_table);
    }

    if (IS_FLAG_SET(this->flags, MULTIBOOT_INFO_BOOT_LOADER_NAME)) {
        e_Log_get()->info("Boot loader: \"{s}\"",
    ...
    if (IS_FLAG_SET(this->flags, MULTIBOOT_INFO_APM_TABLE)) {
        ...
        e_Log_get()->info("APM table: version={iu}; flags={iub}",
...
void e_Multiboot_Info::log_memory_map() {
    ...
    while (iterator.has_next()) {
        ...
        e_Log_get()->info("Memory map region: addr=[{iuh}, {iuh}]; "
...
void e_Multiboot_Info::log_symbol_table() {
    if (IS_FLAG_SET(this->flags, MULTIBOOT_INFO_AOUT_SYMS)) {
        ...
        e_Log_get()->info("a.out symbol table: "
        ...
        e_Log_get()->info("ELF section header table: "
...
void e_Multiboot_Info::log_vbe() {
    if (IS_FLAG_SET(this->flags, MULTIBOOT_INFO_VBE_INFO)) {
        e_Log_get()->info("VBE: control info={iuh}; mode info={iuh}; "
    ...
    if (IS_FLAG_SET(this->flags, MULTIBOOT_INFO_FRAMEBUFFER_INFO)) {
        ...
        e_Log_get()->info("VBE 2.0+ interface table: segment={iuh}; "
